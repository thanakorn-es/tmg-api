		  ...
		  rp.get(''+ config.endpoint.api_lookup.protocol +'://'+ config.endpoint.api_lookup.url +':'+ config.endpoint.api_lookup.port +'/api/lookup/partner/mbcode/' + req.body.MBCODE)
			.then(function(result){
			  ...
			  for (var i = 0; i < limit_; i++) {
				...
				var MBDATT_ = 0;
				if(m_result[0].MBEXP){
					MBEXP_ = m_result[0].MBEXP;
				}
				if(m_result[0].MBPOINT){
					MBPOINT_ = m_result[0].MBPOINT;
				}
				if(m_result[0].MBCEXP){
					MBCEXP_ = m_result[0].MBCEXP;
				}
				if(m_result[0].MBDATT){
					MBDATT_ = m_result[0].MBDATT;
				}
